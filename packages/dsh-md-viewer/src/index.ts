...
export const MDV_GUIDANCE =
  '本机已安装 dsh-md-viewer 插件（MD 文档查看器）：侧边栏/会话标题栏「MD 文档」入口，用 md-viewer 引擎把工作区 Markdown 与 JSON 构建为自包含离线快照（浏览器端按需渲染：marked + highlight.js + mermaid；JSON 以 pretty 文本展示并支持行内展开/折叠，默认 3 层可配置，目录默认隐藏 JSON、工具栏花括号按钮切换显示）并内嵌浏览。能力：mdv_build 构建指定目录（缺省会话工作区，产物在 <目录>/.agents/md-viewer/index.html）、mdv_status 查看已构建快照；构建由用户在 GUI 触发或 Agent 显式调用，以宿主进程权限写入所选目录 .agents/md-viewer/ 下一个自包含 HTML。用户提到「MD 文档 / Markdown 浏览 / md-viewer」时即指本插件，请据此协作。'
