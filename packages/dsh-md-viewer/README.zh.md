...
在 DeepSeek Harness Web GUI 里直接浏览任意工作区的 Markdown 与 JSON——由 **md-viewer 引擎**
...
- **一键构建快照**：选择工作区（或手动输入任意目录路径），点「构建并预览」；
  Host 把目录下所有 `*.md` / `*.json` 渲染进
  `<目录>/.agents/md-viewer/index.html` 并缓存在内存。
...
- 构建**由用户在 GUI 触发**或 **Agent 显式调用**（`mdv_build`）；Host 只写
  用户所选目录下一个文件 `<目录>/.agents/md-viewer/index.html`，使用宿主进程权限。
