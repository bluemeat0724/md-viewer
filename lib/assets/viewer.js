(function () {
  'use strict';
  var META = %%META_JSON%%;
  // 快照数据在 <script type="application/json" id="mdv-data">（构建期已把 < 转义），
  // JSON.parse 读取比 JS 字面量省解析开销；内容是文档原始文本（md/json）
  var SNAPSHOT = (function () {
    try {
      var el = document.getElementById('mdv-data');
  ...
  function attr(s) { return MDV.escAttr(s); }
  ...
  // JSON 文件可见性：默认隐藏（目录与搜索不含 JSON），工具栏 braces 按钮切换
  var showJson = false;
  try { showJson = localStorage.getItem('mdv-show-json') === '1'; } catch (e) {}
  // 渲染结果 LRU 缓存：slug → html（重复打开免重渲染）
  var renderCache = {};
  var renderOrder = [];
  var RENDER_CACHE_MAX = 10;
  // pretty JSON 展开层级：深度 ≥ expand 的容器折叠为 { ⋯ N }，点击行内展开
  var jsonExpand = 3;
  try {
    var savedExpand = parseInt(localStorage.getItem('mdv-json-expand'), 10);
    if (!isNaN(savedExpand) && savedExpand >= 0) jsonExpand = Math.min(savedExpand, 99);
  } catch (e) {}

  /* ---------------- 文档集合状态 ---------------- */
  // 始终使用构建时快照；扫描目录在构建期固定，无需运行时选文件夹。
  var docs = [];               // [{slug,path,group,title,type,raw,size,omitted}]
  var docBySlug = {};
  var docByPath = new Map();   // rel -> {slug}（客户端渲染时链接重写用）
  ...
  function tocItemHtml(d, depth) {
    ...
    var cls = 'toc-item' + (d.type === 'json' ? ' is-json' : '') + (d.omitted ? ' is-omitted' : '');
    return '<button type="button" class="' + cls + '" data-slug="' + attr(d.slug) + '" title="' + attr(d.path) + '" style="--d:' + depth + '">' +
           '<span class="toc-title">' + esc(d.title) + (d.type === 'json' ? ' <span class="toc-badge">JSON</span>' : '') + '</span>' +
           '<span class="toc-path">' + esc(fname) + '</span></button>';
  }
  function buildTree() {
    var root = { name: '', type: 'dir', children: {}, count: 0 };
    docs.forEach(function (d) {
      // JSON 默认不进目录树；切换可见性时整体重建（分组计数随之刷新）
      if (d.type === 'json' && !showJson) return;
  ...
  function renderSidebar() {
    ...
    // 目录被过滤到空（如仅有 JSON 且未开启显示）时给出引导，而不是留白
    if (!html && docs.length) {
      html = '<div class="toc-empty">未显示任何文件。<br>目录默认只列出 Markdown；' +
             '若本目录含 JSON 文件，可点击工具栏的花括号按钮显示。</div>';
    }
  ...
  function docText(d) {
    ...
    // 快照存原始文本，直接作为全文搜索语料（md/json 一致；超限条目为空串）
    var t = d.raw || '';
    textCache[d.slug] = t;
    return t;
  }
  ...
  function docFooterHtml(d) {
    var kind = d.type === 'json' ? 'JSON' : 'Markdown';
    return '<footer class="doc-meta">源文件：<a href="./' + attr(d.path) + '" target="_blank" rel="noopener">' + esc(d.path) +
           '</a>（构建时快照 · 在新标签页打开原始 ' + kind + '）</footer>';
  }

  /* ---------------- 按需渲染（md/json 分发 + LRU 缓存） ---------------- */
  function cacheGet(slug) {
    if (renderCache[slug] === undefined) return undefined;
    var i = renderOrder.indexOf(slug);
    if (i >= 0) renderOrder.splice(i, 1);
    renderOrder.push(slug);
    return renderCache[slug];
  }
  function cachePut(slug, html) {
    var i = renderOrder.indexOf(slug);
    if (i >= 0) renderOrder.splice(i, 1);
    renderCache[slug] = html;
    renderOrder.push(slug);
    while (renderOrder.length > RENDER_CACHE_MAX) {
      delete renderCache[renderOrder.shift()];
    }
  }
  /** 丢弃单个缓存条目（展开层级变化后 JSON 视图整体失效） */
  function cacheDrop(slug) {
    if (renderCache[slug] === undefined) return;
    delete renderCache[slug];
    var i = renderOrder.indexOf(slug);
    if (i >= 0) renderOrder.splice(i, 1);
  }

  /** 渲染一篇文档的正文 HTML（不含 footer）；结果进 LRU 缓存 */
  function renderDocHtml(d) {
    var cached = cacheGet(d.slug);
    if (cached !== undefined) return cached;
    var html;
    if (d.omitted) html = omittedHtml(d);
    else if (d.type === 'json') html = jsonDocHtml(d);
    else html = MDV.renderMarkdown(d.raw, {
      marked: window.marked, hljs: window.hljs, docByPath: docByPath, fromPath: d.path,
    });
    cachePut(d.slug, html);
    return html;
  }

  /** 超限未嵌入的条目：占位提示（保留目录项以便定位与搜索文件名） */
  function omittedHtml(d) {
    var kb = d.size ? (d.size / 1024).toFixed(1) : '?';
    return '<div class="doc-notice">' + icon('alert') +
      '<div><strong>' + esc(d.path) + '</strong> 未嵌入快照：文件 ' + kb + ' KB 超过构建上限。' +
      '可通过下方源文件链接打开原文，或调大 build({ maxRawKB }) 后重建。</div></div>';
  }

  /** JSON 文档：pretty JSON 直接展示 + 行内折叠 + 展开层级输入；解析失败降级原文 */
  function jsonDocHtml(d) {
    var r = MDV.prettifyJson(d.raw);
    if (!r.ok) {
      return '<div class="doc-notice">' + icon('alert') +
        '<div>JSON 解析失败（' + esc(r.error) + '），已按原文显示。</div></div>' +
        '<pre><code class="language-json">' + esc(d.raw) + '</code></pre>';
    }
    return '<div class="json-bar">' +
        '<label class="json-expand">展开层级 ' +
          '<input id="jf-expand" type="number" min="0" max="99" step="1" value="' + jsonExpand + '">' +
        '</label>' +
        '<span class="json-bar-tip">点击括号或 ⋯ 展开 / 折叠</span>' +
      '</div>' +
      '<div class="json-view"><pre><code class="language-json">' +
        MDV.prettyJsonHtml(r.value, jsonExpand) +
      '</code></pre></div>';
  }

  // 行内折叠切换：括号与 ⋯ 标记都可点；closest 命中的是最内层节点，
  // 嵌套节点的切换互不影响
  function toggleJfNode(node) {
    var folded = node.getAttribute('data-folded') === '1';
    var body = node.querySelector('.jf-body');
    var mark = node.querySelector('.jf-mark');
    if (!body || !mark) return;
    if (folded) {
      ...
      node.setAttribute('data-folded', '1');
      body.hidden = true;
      mark.hidden = false;
    }
  }
  docContent.addEventListener('click', function (ev) {
    var t = ev.target;
    if (!t || !t.closest) return;
    var hit = t.closest('.jf-mark') || t.closest('.jf-brace');
    if (!hit) return;
    var node = hit.closest('.jf-node');
    if (node) toggleJfNode(node);
  });

  // 展开层级输入：变更后持久化、令 JSON 渲染缓存失效并重渲染当前文档
  docContent.addEventListener('change', function (ev) {
    if (!ev.target || ev.target.id !== 'jf-expand') return;
    var v = parseInt(ev.target.value, 10);
    if (isNaN(v) || v < 0) v = 3;
    if (v > 99) v = 99;
    if (v === jsonExpand) return;
    jsonExpand = v;
    try { localStorage.setItem('mdv-json-expand', String(v)); } catch (e) {}
    docs.forEach(function (d) { if (d.type === 'json') cacheDrop(d.slug); });
    if (currentSlug && docBySlug[currentSlug] && docBySlug[currentSlug].type === 'json') {
      showDoc(currentSlug, {});
    }
  });
  ...
  function showDoc(slug, opts) {
    ...
    // 经 hash / 上下篇 / 内链落到 JSON 文档时自动开启可见性，侧栏才能定位当前项
    if (d.type === 'json' && !showJson) setJsonVisible(true);
    docContent.innerHTML = renderDocHtml(d) + docFooterHtml(d);
    // JSON 文档无标题结构：隐藏右侧本页目录（含窄屏弹层入口），正文独占全宽
    var noOutline = d.type === 'json';
    document.body.classList.toggle('no-outline', noOutline);
    if (noOutline) {
  ...
  function buildOutline() {
    ...
    Array.prototype.forEach.call(heads, function (h) {
      ...
      html += '<a class="outline-item lvl' + h.tagName.charAt(1) + '" href="#' + attr(h.id) + '" data-id="' + attr(h.id) + '">' + esc((h.textContent || '').trim()) + '</a>';
    });
  ...
  function setJsonVisible(on) {
    showJson = on;
    try { localStorage.setItem('mdv-show-json', on ? '1' : '0'); } catch (e) {}
    var btn = $('btn-json');
    btn.classList.toggle('on', on);
    btn.setAttribute('aria-pressed', on ? 'true' : 'false');
    // 整体重建目录树（分组计数随过滤结果刷新），搜索词若存在则重新应用
    renderSidebar();
    updateFoot();
  }
  $('btn-json').addEventListener('click', function () { setJsonVisible(!showJson); });

  /* ---------------- Mermaid ---------------- */
  // mermaid bundle 以 <script type="text/plain"> 惰性内嵌（无 mermaid 文档时
  // 构建期直接省略），首次渲染图表时才激活，免去启动即解析约 3.4MB
  function ensureMermaidLoaded() {
    if (typeof window.mermaid !== 'undefined') return;
    var el = document.getElementById('mermaid-src');
    if (!el) return;
    try { (0, eval)(el.textContent); } catch (e) {}
  }
  function mmTheme() {
  ...
  function updateFoot() {
    var mdCount = 0, jsonCount = 0;
    docs.forEach(function (d) { if (d.type === 'json') jsonCount++; else mdCount++; });
    var text = '共 ' + mdCount + ' 篇文档';
    if (jsonCount) text += NL + jsonCount + ' 个 JSON 文件（' + (showJson ? '显示中' : '已隐藏，工具栏花括号可开启') + '）';
    text += NL + '快照生成于 ' + META.generatedAt;
    $('side-foot').textContent = text;
  }
  ...
  /* ---------------- 启动 ---------------- */
  docs = SNAPSHOT.map(function (d) {
    return {
      slug: d.slug, path: d.path, group: d.group, title: d.title,
      type: d.type || 'md', raw: d.raw, size: d.size, omitted: d.omitted,
    };
  });
  rebuildIndexes();
  $('btn-json').classList.toggle('on', showJson);
  $('btn-json').setAttribute('aria-pressed', showJson ? 'true' : 'false');
  renderSidebar();
  updateFoot();
  applyTheme();
  // 恢复桌面端侧边栏折叠偏好（移动端忽略）
